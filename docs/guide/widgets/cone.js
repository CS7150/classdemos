...
var Q=[[0.40,0.85],[0.40,-0.85],[0.55,0.35],[0.45,-0.45],[0.60,0.95],[0.75,0.10],[0.42,-0.20],[0.50,0.55]];
var XS=[],P=[],G=9,RR2=0;
for(var z=0;z<Q.length;z++){var qa=Q[z][0]*W[0]+Q[z][1]*U[0],qb=Q[z][0]*W[1]+Q[z][1]*U[1];XS.push([qa,qb]);P.push([qa,qb,1]);P.push([-qa,-qb,-1]);
 G=Math.min(G,Q[z][0]);RR2=Math.max(RR2,Math.hypot(qa,qb));}
var RAD=RR2,KB=Math.floor(RAD*RAD/(G*G));
...
function anim(){var tl=Math.max(42,Math.min(94,126/need())),tr=Math.max(14,Math.min(90,120/need()));
...
function render(){
 ...
 var ang=n>0?Math.acos(Math.max(-1,Math.min(1,(W[0]*w[0]+W[1]*w[1])/n)))*180/Math.PI:null;
 document.getElementById('readout').innerHTML=msg+'<br>mistakes so far <b>k = '+k+'</b> &middot; misclassified <b>'+nb+' of 16</b>'+
  (ang!=null?' &middot; angle of w to w* <b>'+ang.toFixed(1)+'&deg;</b>':'');
}
